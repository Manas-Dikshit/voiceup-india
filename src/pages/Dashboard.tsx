...
const Dashboard = () => {
  ...
  // fetch current user's votes for visible problems so we can show active vote state
  const problemIds = (Array.isArray(problems) ? problems : []).map((p: Problem) => p.id).filter(Boolean);
  const { data: myVotes = [], isFetching: myVotesLoading } = useQuery({
    queryKey: ['myVotes', profile?.id, ...problemIds],
    queryFn: async () => {
      if (!profile || problemIds.length === 0) return [];
      // supabase types in this workspace don't include 'votes' in the autogenerated DB types,
      // cast to any to avoid TS overload issues for this runtime query.
    ...
    enabled: !!profile && problemIds.length > 0,
  });

  const userVoteMap: Record<string, 'upvote'|'downvote' | undefined> = {};
  (Array.isArray(myVotes) ? myVotes : []).forEach((v: any) => {
    if (v && v.votable_id) userVoteMap[v.votable_id] = v.vote_type;
  });
